...
<div class=" container">
	<ul class="pagination">
		...
		<c:if test="${requestScope.pageNo!=1}">
			<li><a href="http://localhost:8080/nw/PageServlet?username=${requestScope.username}&password=${requestScope.password}&pageNo=${requestScope.pageNo-1}&action=动漫区">&laquo;</a></li>
		</c:if>
		<c:forEach begin="${requestScope.pageNo}" end="${requestScope.pageNo+5}" var="i">
			...
			<c:if test="${i<=requestScope.pageTotal&&i!=requestScope.pageNo}">
				<li><a href="http://localhost:8080/nw/PageServlet?username=${requestScope.username}&password=${requestScope.password}&pageNo=${i}&action=动漫区">${i}</a></li>
			</c:if>
		</c:forEach>
		<c:if test="${requestScope.pageNo!=requestScope.pageTotal}">
			<li><a href="http://localhost:8080/nw/PageServlet?username=${requestScope.username}&password=${requestScope.password}&pageNo=${requestScope.pageNo+1}&action=动漫区">&raquo;</a></li>
		</c:if>

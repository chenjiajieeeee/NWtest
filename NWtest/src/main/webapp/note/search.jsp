...
<div class=" container">
    <ul class="pagination">
        ...
        <c:if test="${requestScope.pageNo!=1}">
            <li><a href="http://localhost:8080/nw/SearchServlet?username=${requestScope.username}&password=${requestScope.password}&pageNo=${requestScope.pageNo-1}&tag=${requestScope.tag}">&laquo;</a></li>
        </c:if>
        <c:forEach begin="${requestScope.pageNo}" end="${requestScope.pageNo+5}" var="i">
            ...
            <c:if test="${i<=requestScope.pageTotal&&i!=requestScope.pageNo}">
                <li><a href="http://localhost:8080/nw/SearchServlet?username=${requestScope.username}&password=${requestScope.password}&pageNo=${i}&tag=${requestScope.tag}">${i}</a></li>
            </c:if>
        </c:forEach>
        <c:if test="${requestScope.pageNo!=requestScope.pageTotal}">
            <li><a href="http://localhost:8080/nw/SearchServlet?username=${requestScope.username}&password=${requestScope.password}&pageNo=${requestScope.pageNo+1}&tag=${requestScope.tag}">&raquo;</a></li>
        </c:if>

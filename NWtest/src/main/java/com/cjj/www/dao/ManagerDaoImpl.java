...
public class ManagerDaoImpl implements ManagerDao{
    ...
    public boolean managerLogin(String account, String password) {
        ...
        String sql="select * from area_administrator where account =  '"+account+"'";
        connection= JdbcUtil.getConnection();
        try {
            statement=connection.createStatement();
            resultSet=statement.executeQuery(sql);
            while (resultSet.next()){
                String checkpassword=resultSet.getNString("password");
    ...
    @Override
    public List<Note> queryNoteByArea(String zoomName) {
        NoteDao noteDao=new NoteDaoImpl();
        List<Note> notes = noteDao.queryNoteByZoom(zoomName);
        return notes;
    }
    ...
    @Override
    public String searchZoomByAccount(String account) {
        Connection connection=null;
        Statement statement=null;
        ResultSet resultSet =null;
        connection=JdbcUtil.getConnection();
        String zoomName=null;
        String sql="select zoom_name from area_administrator where account = '"+account+"'";
        try {
            statement=connection.createStatement();
            resultSet=statement.executeQuery(sql);
            while (resultSet.next()){
                zoomName=resultSet.getString("zoom_name");
            }
        } catch (SQLException throwables) {
            throwables.printStackTrace();
        } finally {
